...
OUTPUT_DIR = '.'
GROUP_FILTER = None  # include both GM and Rural.Hadza
...
def build_pipeline(df):
    ...
    return clf, X, y, cat_features, num_features
...
def save_outputs(model, results):
    import pickle
...
def main():
    print(f"Running OHE XGBoost for age group 18-24 (16-20 + 21-24)")
    df = load_and_filter()
    if df.empty:
        print('No data after filtering. Exiting.')
        return
    clf, X, y, cat_features, num_features = build_pipeline(df)
    model, results = tune_and_train(clf, X, y)
