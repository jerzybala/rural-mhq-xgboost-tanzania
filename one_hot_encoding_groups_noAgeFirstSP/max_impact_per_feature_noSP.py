"""
For each feature, find the group with the highest mean absolute SHAP (no AgeOfFirstSP/Smartphone.ownership).
Outputs a CSV and a bar plot colored by winning group.
"""
...
OUTPUT_CSV = "max_impact_per_feature_noSP.csv"
OUTPUT_PNG = "max_impact_per_feature_noSP.png"
TOP_N = 40  # number of features to show (sorted by best mean SHAP)
...
def plot_max(df: pd.DataFrame, outfile: str, top_n: int):
    ...
    plt.title(f"Per-feature highest mean SHAP (noSP) — top {top_n}")
