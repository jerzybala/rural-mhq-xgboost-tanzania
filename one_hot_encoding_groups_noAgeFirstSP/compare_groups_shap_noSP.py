"""
Compare SHAP mean absolute values between one-hot group runs (no AgeOfFirstSP).
...
OUTPUT_EXCEL = "group_shap_comparison_noSP.xlsx"
PLOT_TOP_N = 20
...
def plot_top_diffs(df_diff: pd.DataFrame, title: str, outfile: str):
    ...
    plt.barh(top["Feature"], top["diff"], color=["#4CAF50" if v > 0 else "#F44336" for v in top["diff"]])
...
def main():
    ...
    with pd.ExcelWriter(os.path.join(base_dir, OUTPUT_EXCEL)) as writer:
        ...
        for a, b in itertools.combinations(GROUP_DIRS.keys(), 2):
            ...
            title = f"Top {PLOT_TOP_N} SHAP mean differences (noSP): {a} - {b}"
            outfile = os.path.join(base_dir, f"shap_diff_{a}_vs_{b}_noSP.png")
