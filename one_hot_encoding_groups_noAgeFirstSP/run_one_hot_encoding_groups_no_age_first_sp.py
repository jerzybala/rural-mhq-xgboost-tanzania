"""
One-hot XGBoost group runs excluding AgeOfFirstSP/AgeOfFirstSmartPhone.
Outputs land in one_hot_encoding_groups_noAgeFirstSP/<group_name>/ with _noSP suffixes.
"""
...
def _tags(job_name: str) -> Tuple[str, str]:
    title_tag = f"{job_name} (no SP)"
    file_tag = f"{job_name}_noSP".replace("/", "_").replace(" ", "_")
...
def plot_and_save(job_dir, job_name, feature_importance, shap_values, shap_importance, explainer, expected_value, X_test, y_test, cm, y_prob, roc_auc):
    ...
    plt.title(f"SHAP Summary Plot (One-Hot) - {title_tag}")
    plt.tight_layout()
    ...
    plt.title(f"SHAP Feature Importance (One-Hot) - {title_tag}")
    plt.tight_layout()
    ...
    for feature in top_features:
        ...
        plt.title(f"SHAP Dependence Plot: {feature} (One-Hot) - {title_tag}")
        plt.tight_layout()
    ...
    # Threshold sweep for precision/recall/F1 with Succeeding as the positive class
...
def save_outputs(job_dir: str, model, feature_importance, shap_importance, metrics):
    ...
    with open(os.path.join(job_dir, "xgboost_model_onehot_noSP.pkl"), "wb") as f:
        pickle.dump(model, f)
    ...
    with open(os.path.join(job_dir, "one_hot_encoding_results_noSP.pkl"), "wb") as f:
        pickle.dump(results, f)

    with open(os.path.join(job_dir, "metrics_noSP.json"), "w") as f:
        json.dump(metrics, f, indent=2)


def run_job(job: Job):
    print(f"\n=== Running job: {job.name} (no AgeOfFirstSP) ===")
