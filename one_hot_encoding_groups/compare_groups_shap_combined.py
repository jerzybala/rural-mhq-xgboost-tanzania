"""
Combined SHAP mean difference plot across all one-hot group pairs (with AgeOfFirstSP/Smartphone.ownership included).
Generates a single PNG and CSV showing per-feature differences for all pairs.
"""
...
PLOT_TOP_N = 25
COMBINED_PNG = "shap_diff_combined.png"
COMBINED_CSV = "shap_diff_combined.csv"
...
def plot_combined(df: pd.DataFrame, outfile: str, top_n: int):
    ...
    # light horizontal separators per feature
    for i in range(len(df_top["Feature"].unique())):
        ax.axhline(i + 0.5, color="#cccccc", linewidth=0.5, zorder=0)
    plt.title(f"SHAP mean differences across groups (one-hot) — top {top_n} features")
    plt.ylabel("Feature")
